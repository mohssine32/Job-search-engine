...
const HeroSection: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (search.trim()) {
      navigate(`/jobpage?search=${encodeURIComponent(search)}`);
    } else {
      navigate('/jobpage');
    }
  };
  ...
  return (
    <div className="h-auto md:h-[600px] bg-[#F8D3E8] flex flex-col justify-center items-center px-4 py-12 md:py-0">
      {/* TITRE */}
      ...
      {/* BARRE DE RECHERCHE */}
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row max-w-3xl w-full mb-8 sm:mb-16 shadow-2xl">
        <div className="relative flex-1 mb-4 sm:mb-0">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 sm:py-4 text-gray-700 bg-white rounded-l-lg border-none outline-none focus:ring-0"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
        </div>
        <button type="submit" className="bg-black text-white px-6 sm:px-8 py-3 sm:py-4 rounded-r-lg hover:bg-gray-800 transition-colors font-medium flex items-center justify-center gap-2">
